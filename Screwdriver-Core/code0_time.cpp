...
namespace TM
{
	struct code0_time::code0_time_imp_t
	{
		code0_time_imp_t()
			:time_(0),
			 data_buffer_ptr_(new data_buffer)
		{
			...
			millisecond_parameter_ptr_=create_base_tm_parameter("big_endian_uint_parameter32", "ms");
	...
	code0_time::code0_time(void)
		:imp_(new code0_time_imp_t)
	{
	}
	...
	double code0_time::get_time() const
	{
		return imp_->time_;
	}

	void code0_time::read_form_buffer(const std::vector<uint8_t>& buffer, uint32_t start_idx)
	{
		imp_->data_buffer_ptr_->read_from_buffer(buffer,start_idx);
		imp_->time_=imp_->second_parameter_ptr_->get_val()+imp_->millisecond_parameter_ptr_->get_val()/1000;		imp_->time_charged_(imp_->time_);
	}

	boost::signals2::connection code0_time::connect_time_charged_signal(const time_charged_slot_t& slot)
	{
